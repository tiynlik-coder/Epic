...
async def admin_guard_message(update, ctx):
    ...
    if c and c.type in {ChatType.GROUP, ChatType.SUPERGROUP}:
        _register_group(c)
        if u and not is_bot_admin(u.id) and (_blocked_group(c.id) or _blocked_user(u.id)):
            raise ApplicationHandlerStop
    elif u and not is_bot_admin(u.id) and _blocked_user(u.id):
        raise ApplicationHandlerStop
...
async def admin_guard_callback(update, ctx):
    ...
    if c and c.type in {ChatType.GROUP, ChatType.SUPERGROUP} and u and not is_bot_admin(u.id) and (_blocked_group(c.id) or _blocked_user(u.id)):
        await q.answer("🚫 Siz uchun bu funksiya bloklangan.", show_alert=True)
        raise ApplicationHandlerStop
    if u and not is_bot_admin(u.id) and _blocked_user(u.id):
